...
function getVolumetricDivisor(destination?: string, country?: string): number {
  const text = `${destination || ""} ${country || ""}`.toUpperCase();
...
function phonesMatch(stored: unknown, inputDigits: string): boolean {
  const p = digitsPhone(stored);
  if (!p || inputDigits.length < 8) return false;
  const last10 = inputDigits.slice(-10);
  return (
    p === inputDigits ||
    (last10.length >= 8 &&
      (p.endsWith(last10) || last10.endsWith(p.slice(-10))))
  );
}
...
async function findPartyByPhone(
  ...
  if (phoneDigits.length < 8) return undefined;
  ...
  const variants = [
    ...
    `+91${last10}`,
    `91${last10}`,
  ].filter((v, i, a) => v.length >= 8 && a.indexOf(v) === i);

  for (const field of ["phone", "mobile"] as const) {
    for (const v of variants) {
      const snap = await col.where(field, "==", v).limit(1).get();
      if (!snap.empty) return snap.docs[0];
    }
  }

  const all = await col.limit(800).get();
  return all.docs.find((d) => {
    const data = d.data() || {};
    return (
      phonesMatch(data.phone, phoneDigits) ||
      phonesMatch(data.mobile, phoneDigits)
    );
  });
}
...
async function upsertSenderFromShipper(
  ...
  const existingId = str(shipper.senderId) || str(shipper.id);
  if (existingId && existingId !== "WALKIN_SENDER") {
    const ref = adminDb
      .collection(FIRESTORE_COLLECTIONS.SENDERS || "senders")
      .doc(existingId);
    const doc = await ref.get();
    if (doc.exists) return { senderId: doc.id, created: false };
  }
  ...
  const col = adminDb.collection(FIRESTORE_COLLECTIONS.SENDERS || "senders");
  const now = new Date().toISOString();
  const match = await findPartyByPhone(col, phoneDigits);
  ...
  const payload = {
    ...
    contactName: str(shipper.contactName) || name || null,
    phone: phone || "0000000000",
    mobile: str(shipper.mobile) || phone || null,
    email: str(shipper.email) || null,
    address: partyAddress(shipper) || null,
    addressLine1: str(shipper.addressLine1) || partyAddress(shipper) || null,
    addressLine2: str(shipper.addressLine2) || null,
    ...
    documentType: str(shipper.documentType) || null,
    documentNo: str(shipper.documentNo) || null,
    documentUrl: str(shipper.documentUrl) || null,
    country: str(shipper.country, "India") || null,
    origin: str(shipper.origin) || null,
    updatedAt: now,
    updatedBy: userId,
...
async function upsertReceiverFromConsignee(
  ...
  const existingId = str(consignee.receiverId) || str(consignee.id);
  if (existingId && existingId !== "WALKIN_RECEIVER") {
    const ref = adminDb
      .collection(FIRESTORE_COLLECTIONS.RECEIVERS || "receivers")
      .doc(existingId);
    const doc = await ref.get();
    if (doc.exists) return { receiverId: doc.id, created: false };
  }
  ...
  const payload = {
    ...
    contactName: str(consignee.contactName) || name || null,
    phone: phone || "0000000000",
    mobile: str(consignee.mobile) || phone || null,
    email: str(consignee.email) || null,
    address: partyAddress(consignee) || null,
    addressLine1:
      str(consignee.addressLine1) || partyAddress(consignee) || null,
...
export async function POST(request: NextRequest) {
  try {
    ...
    const shipper: Record<string, unknown> = { ...(body.shipper || {}) };
    const consignee: Record<string, unknown> = { ...(body.consignee || {}) };

    if (body.senderId) shipper.senderId = body.senderId;
    if (body.receiverId) consignee.receiverId = body.receiverId;
