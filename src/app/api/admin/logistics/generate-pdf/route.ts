...
//             // Manifest total = shipment value ONLY (not proforma Box1+Box2)
//       const shipmentValueOnly = Number(
//         body.shipmentValue ??
//           body.shipment_value ??
//           body.chargesGrandTotal ??
//           0,
//       );
...
//     // Proforma total = Box 1 + Box 2 line amounts (label declared value)
//     const proformaItemsTotal = items.reduce(
//       (sum, it) => sum + Number(it.amount || 0),
//       0,
//     );
...
//       ),
//       dimensions: body.dimensions || "",
//       // Prefer sum of proforma lines (Box 1 + Box 2); fallback body.declaredValue
//       declaredValue:
//         proformaItemsTotal > 0
//           ? proformaItemsTotal
//           : Number(body.declaredValue || body.totalAmount || 0),
//       currency: body.currency || "INR",
//       totalAmount:
//         proformaItemsTotal > 0
//           ? proformaItemsTotal
...
//         chargeableWeight: common.chargeableWeight,
//         dimensions: common.dimensions,
//         // Proforma items total (all boxes) — not freight / charges
//         declaredValue: common.declaredValue,
//         currency: common.currency,
//         content: common.content,
//         csbType: common.csbType,
...
//     // Proforma total = Box 1 + Box 2 (AWB label declared value)
//     const proformaItemsTotal = items.reduce(
//       (sum, it) => sum + Number(it.amount || 0),
//       0,
//     );
...
//     // Proforma only — never invent content when there are no line items
//     const contentText =
//       contentFromItems ||
//       (items.length > 0
//         ? String(body.content || body.description || "").trim()
...
//       //     ? proformaItemsTotal
//       //     : Number(body.declaredValue || body.totalAmount || 0),
//       declaredValue: proformaItemsTotal,
//       currency: body.currency || "INR",
//       // totalAmount:
//       //   proformaItemsTotal > 0
//       //     ? proformaItemsTotal
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (type === "manifest") {
      ...
      // Shipping cost ONLY — never proforma items total
      const shipmentValueOnly = Number(
        body.shipmentValue ??
          body.shipment_value ??
          body.chargesGrandTotal ??
