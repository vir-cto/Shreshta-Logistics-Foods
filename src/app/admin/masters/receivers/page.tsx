...
type SortKey = "companyName" | "phone" | "city" | "country" | "status";
...
export default function ReceiversPage() {
  ...
  async function handleDelete(row: Receiver) {
    if (!firebaseUser) return;
    const label = row.companyName || row.contactName || row.name;
    if (
      !window.confirm(
        `Delete receiver "${label}" permanently?\n\nThis cannot be undone.`,
      )
    ) {
      return;
    }
    ...
    try {
      setDeletingId(row.receiverId);
      setError(null);
      setMessage(null);
      const token = await firebaseUser.getIdToken(true);
      const res = await fetch(
        `/api/logistics/receivers?receiverId=${encodeURIComponent(row.receiverId)}`,
        {
          method: "DELETE",
          headers: {
            Accept: "application/json",
      ...
      const json = await res.json();
      if (!res.ok || !json.success) {
        throw new Error(json?.error?.message || "Failed to delete receiver.");
      }
      setMessage("Receiver deleted.");
      setReloadKey((v) => v + 1);
    } catch (e) {
      setError(e instanceof Error ? e.message : "Failed to delete receiver.");
    } finally {
      setDeletingId(null);
    }
  }
                    ...
                    <td className="px-4 py-3 text-right">
                      <div className="flex justify-end gap-2">
                        ...
                        <button
                          ...
                          disabled={deletingId === row.receiverId}
                          className="rounded p-1.5 text-slate-400 hover:bg-red-50 hover:text-red-600 disabled:opacity-50"
                          title={
                            deletingId === row.receiverId
                              ? "Deleting…"
                              : "Delete receiver"
                          }
                          aria-label={`Delete ${row.companyName || row.name}`}
                        >
