...
export default function LogisticsSettingsPage() {
  ...
  const canManageSettings = can(permissionUser, "LOGISTICS_SETTINGS");
  ...
  useEffect(() => {
    ...
    async function loadSettings() {
      try {
        ...
        if (!can(permissionUser, "LOGISTICS_SETTINGS")) {
          throw new Error(
            "You do not have permission to manage settings (LOGISTICS_SETTINGS).",
          );
        }
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!canManageSettings) {
        throw new Error(
          "You do not have permission to save settings (LOGISTICS_SETTINGS).",
        );
      }
      ...
      setSettings(normalizeLoadedSettings(json.data));
      setDirty(false);
      const okMsg = json.message || "Logistics Settings saved successfully.";
      setMessage(okMsg);
      setToast(okMsg);
      ...
      <div className="mx-auto max-w-[1100px]">
        <div className="rounded-xl border border-red-200 bg-red-50 p-6 shadow-sm">
          ...
          <p className="mt-1 text-sm text-red-800">
            Your role is <strong>{roleLabel}</strong>. Logistics settings
            require{" "}
            <code className="font-mono">LOGISTICS_SETTINGS</code>.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="mx-auto max-w-[1100px]">
      ...
      <div className="mt-5 rounded-lg border border-amber-200 bg-amber-50 p-4 text-xs text-amber-800">
        Requires{" "}
        <code className="font-mono">LOGISTICS_SETTINGS</code>. Settings
        are stored in Firestore; client UI never bypasses server
        authorization.
      </div>
    </div>
  );
