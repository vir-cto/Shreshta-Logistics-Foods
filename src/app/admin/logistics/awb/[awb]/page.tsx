...
//           declaredValue: (detail.items || []).reduce(
//             (s, it) => s + Number(it.amount || 0),
//             0,
//           ),
//           currency: "INR",
...
//           //   ) ||
//           //   0,
//           totalAmount: (detail.items || []).reduce(
//   (s, it) => s + Number(it.amount || 0),
//   0,
// ),
//           forwardingNo: detail.awb,
...
//   // const fuelSurcharge = Number(charges.fuelSurcharge || 0);
//   // const otherCharges = Number(
//   //   charges.otherCharges || charges.contractCharges || 0,
//   // );
//   // const tax = Number(
//   //   charges.igst ||
//   //     charges.tax ||
...
//   // // Prefer stored total; otherwise sum line charges + additionalCharges
//   // // let additionalSum = 0;
//   // // if (Array.isArray(charges.additionalCharges)) {
//   // //   additionalSum = (charges.additionalCharges as unknown[]).reduce(
//   // //     (s, row) => {
//   // //       const r = asRecord(row) || {};
//   // //       return s + Number(r.total ?? r.amount ?? 0);
//   // //     },
//   // //     0,
//   // //   );
//   // // }
...
//   //   let additionalSum = 0;
//   // if (Array.isArray(charges.additionalCharges)) {
//   //   for (const row of charges.additionalCharges) {
//   //     if (!row || typeof row !== "object") continue;
//   //     const r = row as Record<string, unknown>;
//   //     additionalSum += Number(r.total ?? r.amount ?? 0) || 0;
//   //   }
//   // }

//   // const total = Number(
//   //   charges.total ||
//   //     freight + fuelSurcharge + otherCharges + tax + additionalSum,
//   // );
...
//   const taxFromGst = Number(gst.totalTax || 0);
//   const tax = Number(
//     charges.tax || taxFromGst || igst || cgst + sgst || 0,
//   );
...
//   // Always sum what we show (do not trust a stale charges.total alone)
//   const computedTotal =
//     freight + fuelSurcharge + otherCharges + tax + additionalSum;
//   const storedTotal = Number(charges.total || 0);
//   // Prefer computed so UI lines match Total; fall back to stored if all lines are 0
//   const total =
//     computedTotal > 0 ? computedTotal : storedTotal > 0 ? storedTotal : 0;
...
//     content: text(shipment.content || shipment.description, ""),
//     instruction: text(
//       shipment.instruction || shipment.specialInstructions || "",
//       "",
//     ),
//     csbType: text(shipment.csbType, ""),
//     exportReason: text(shipment.exportReason, ""),
...
//       // Proforma goods total (label / proforma only)
//       const proformaTotal = (detail.items || []).reduce(
//         (s, it) => s + Number(it.amount || 0),
//         0,
//       );
//       // Shipping cost = Charges total on this page (freight + fuel + other + GST)
//       const shipmentCost = Number(detail.totalRaw) || 0;

//       const contentFromItems = (detail.items || [])
//         .map((it) => {
//           const d = String(it.description || "").trim();
//           const s = String(it.shopName || "").trim();
//           if (d && s) return `${d} (${s})`;
...
//           // Manifest “Shipment Value” = shipping cost only
//           shipmentValue: shipmentCost,
//           chargesGrandTotal: shipmentCost,
//           forwardingNo: detail.awb,
//           manifestNo: detail.accountCode || detail.awb,
//         }),
//       });
...
function normalizeShipment(
  ...
  // Sum of displayed charge lines (prefer this over a stale charges.total)
  const computedTotal =
    freight + fuelSurcharge + otherCharges + tax + additionalSum;
  const storedTotal = Number(charges.total || 0);
  const total =
  ...
  // Declared value = proforma goods only (e.g. ₹2,800) — NOT shipping cost
  const declaredValueNum = items.reduce(
    (s, it) => s + Number(it.amount || 0),
    0,
  );
...
export default function AWBDetailPage() {
  ...
  async function generatePdf(
    ...
    try {
      ...
      // Shipping cost = Charges total (freight + fuel + other + GST)
      const shipmentCost = Number(detail.totalRaw) || 0;
